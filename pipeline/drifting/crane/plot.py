...
def plot_mean_sessions(freq, sessions_mean, smooth_box, bsl_flag=True, polyfit_deg=1):
    ...
    ax1.plot(freq, sessions_mean, linewidth=1, color='black')
    ax1.plot(freq, bsl_curv, alpha=0.6, color='orange', linewidth=2)
    ...
    ax3.plot(freq, sessions_mean_bsl, linewidth=1, color='black')
...
def plot_substract(freq, sessions_mean, start_i, end_i):
    ...
    plt.plot(freq, sessions_mean, color='black', linewidth=1)
    axes = plt.gca()
